...
import org.sonarqube.ws.client.WsClientFactory; // Import WsClientFactory
...
class PrometheusWebServiceTest {
    ...
    @Mock WsClientFactory mockWsClientFactory; // Mock the factory returned by getLocal()
    ...
    void setUp() {
        ...
        // WsClientFactories.getLocal() returns our mock WsClientFactory
        mockedStaticWsClientFactories.when(WsClientFactories::getLocal).thenReturn(mockWsClientFactory);
        when(mockWsClientFactory.newClient(any(WsConnector.class))).thenReturn(wsClient); // newClient is called on WsClientFactory with LocalConnector
    ...
    void handle_whenNoMetricsEnabled_shouldReturnEmptyMetrics() throws Exception {
        ...
        verifyNoInteractions(mockWsClientFactory); // Verify on the correct factory mock
    ...
    void handle_whenMetricsEnabled_shouldFetchAndExportNumericAndAlertStatusMetrics() throws Exception {
        ...
        verify(mockWsClientFactory).newClient(any(WsConnectorJava.class)); // Verify newClient call on the WsClientFactory mock
